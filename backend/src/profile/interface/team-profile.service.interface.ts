...
/**
 * 팀 프로필 이미지를 관리하는 서비스 인터페이스
 * @extends {ProfileService} 이미지를 관리하는 기본 인터페이스
 */
export interface TeamProfileService extends ProfileService {
  /**
   * 팀 프로필 이미지를 등록 또는 업데이트하고 프로필 이미지가 저장된 url을 반환합니다.
   *
   * @param {Express.Multer.File} image - 이미지 파일
   * @param {number | string} teamId  - 프로필 이미지를 등록할 엔티티의 식별자
