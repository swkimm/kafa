...
import type { FileStorageService } from '../interface/file-storage.service.interface'

/**
 * Object Storage의 파일을 관리하는 서비스 인터페이스 [FILEStorageService]의 구현체.
...
export class FileStorageServiceImpl implements FileStorageService {
  ...
  async deleteObject(originUrl: string): Promise<{ result: string }> {
    try {
      const originKey = this.extractFilePathFromUrl(originUrl)
